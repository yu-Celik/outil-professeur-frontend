          ...
          },
          {
            ...
            levels: [
              ...
              },
              {
                ...
                name: "Bien",
                description:
                  "Bonne prononciation avec quelques erreurs mineures",
                points: 3,
              },
              {
                id: "satisfactory",
...
export function validateRubric(rubric: Rubric): {
  isValid: boolean;
  errors: string[];
} {
  const errors: string[] = [];
  ...
  const totalWeight = Object.values(sections).reduce(
  ...
  if (totalCriteria < constraints.minCriteria) {
    errors.push(
      `Nombre minimum de critères: ${constraints.minCriteria} (actuel: ${totalCriteria})`,
    );
  }
  if (totalCriteria > constraints.maxCriteria) {
    errors.push(
      `Nombre maximum de critères: ${constraints.maxCriteria} (actuel: ${totalCriteria})`,
    );
  }
  ...
  if (maxScore !== constraints.maxTotalPoints) {
    errors.push(
      `Score maximum calculé (${maxScore}) ne correspond pas au maximum défini (${constraints.maxTotalPoints})`,
    );
  }
