...
export function useRubricManagement(
  ...
  const createRubric = useCallback(
    async (data: RubricFormData): Promise<Rubric> => {
      try {
      ...
      try {
        ...
        const updated: Rubric = {
          ...
          constraints: (data.constraints ||
            rubrics[existingIndex].constraints) as unknown as Record<
            string,
            unknown
          >,
          updatedAt: new Date(),
  ...
  const validateRubricData = useCallback(
    (rubric: Rubric): RubricValidationResult => {
