...
export function useExamFilters(
  ...
  const filteredExams = useMemo(() => {
    ...
    if (
      ...
      filters.academicPeriodId !== "ALL_PERIODS"
    ) {
      result = result.filter(
        (exam) => exam.academicPeriodId === filters.academicPeriodId,
      );
    }
    ...
    if (filters.dateRange.start) {
      result = result.filter(
        (exam) => exam.examDate >= filters.dateRange.start!,
      );
    }
  ...
  const updateFilter = useCallback(
    <K extends keyof ExamFilters>(key: K, value: ExamFilters[K]) => {
      setFilters((prev) => ({
        ...prev,
        [key]: value,
      }));
  ...
  const updateSort = useCallback(
    (field: ExamSortField, direction?: SortDirection) => {
      setSort((prev) => ({
        field,
        direction:
          direction ||
  ...
  const classIds = useMemo(() => {
    ...
    const filteredClasses = Array.from(examClasses).filter((classId) =>
      authorizedClasses.has(classId),
    );
