...
export function useGradeManagement(teacherId: string) {
      ...
      try {
        ...
        if (
          ...
          if (!validateGrade(gradeData.grade)) {
            return {
              success: false,
              error: "Note invalide selon le système de notation",
            };
          }
        }
  ...
  const calculateStatistics = useCallback(
    (results: StudentExamResult[]): GradeStatistics => {
      const validResults = results.filter(
        (r) => !r.isAbsent && r.grade !== null,
      );
  ...
  const calculateGradeFromPoints = useCallback(
    (pointsObtained: number, exam: Exam): number => {
  ...
  const getGradeHistory = useCallback(
    (resultId: string): GradeChangeHistory[] => {
    ...
    (
      ...
      return grades.map(({ grade, isAbsent }) => {
        ...
        if (grade === null || grade === undefined)
          return { isValid: false, error: "Note manquante" };
        if (!validateGrade(grade))
          return { isValid: false, error: "Note invalide" };
        return { isValid: true };
      });
    },
    [validateGrade],
  ...
  return {
    ...
    formatGrade: (grade: number) =>
      defaultSystem ? formatGrade(grade, defaultSystem) : grade.toString(),
  };
}
