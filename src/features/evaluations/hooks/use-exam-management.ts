...
export function useExamManagement(
        ...
        const newExam: Exam = {
          ...
          addResult: function (
            ...
            const notationSystem =
              notationSystems.find((ns) => ns.id === this.notationSystemId) ||
              defaultSystem;

            const computedGrade = notationSystem
              ? calculateGradeFromPoints(points, this, notationSystem)
              : points;
  ...
  const updateExam = useCallback(
    async (examId: string, data: Partial<ExamFormData>): Promise<Exam> => {
      try {
  ...
  const deleteExam = useCallback(async (examId: string): Promise<void> => {
    try {
      ...
      const resultIndices = MOCK_STUDENT_EXAM_RESULTS.map((result, index) =>
        result.examId === examId ? index : -1,
      )
  ...
  const toggleExamPublication = useCallback(
  ...
  const addExamResult = useCallback(
    async (
      examId: string,
      data: StudentExamResultFormData,
      ...
      try {
        ...
        const computedPoints = data.isAbsent ? 0 : data.pointsObtained;
        const computedGrade = notationSystem
          ? calculateGradeFromPoints(computedPoints, exam, notationSystem)
          : computedPoints;
  ...
  const updateExamResult = useCallback(
    async (
      resultId: string,
      data: Partial<StudentExamResultFormData>,
    ): Promise<StudentExamResult> => {
      try {
        const resultIndex = MOCK_STUDENT_EXAM_RESULTS.findIndex(
          (r) => r.id === resultId,
        );
        if (resultIndex === -1) {
        ...
        if (!exam) {
          throw new Error("Examen introuvable pour ce résultat");
        }
        ...
        const nextPoints = data.isAbsent
          ? 0
          : (data.pointsObtained ?? existing.pointsObtained);

        const nextGrade = system
          ? calculateGradeFromPoints(nextPoints, exam, system)
          : nextPoints;

        const updatedResult: StudentExamResult = {
          ...existing,
          pointsObtained: nextPoints,
          isAbsent: Boolean(data.isAbsent),
          comments: data.comments ?? existing.comments,
          grade: data.isAbsent ? (system?.minValue ?? 0) : nextGrade,
          gradeDisplay: data.isAbsent
            ? "ABS"
            : system
              ? formatGradeDisplay(nextGrade, system, "fr-FR")
              : nextGrade.toString(),
          markedAt: new Date(),
          isPassing(system) {
            if (this.isAbsent) return false;
          ...
          gradeCategory(system) {
            ...
            if (ratio >= 60) return "Bien";
            if (ratio >= 40) return "Satisfaisant";
            return "Insuffisant";
          },
  ...
  const getExamsBySubjectId = useCallback(
    (subjectId: string) => {
  ...
  const getExamsByAcademicPeriodId = useCallback(
    (academicPeriodId: string) => {
  ...
  const validateResultForm = useCallback(
    (data: StudentExamResultFormData, examId: string) => {
      ...
      if (!data.isAbsent) {
        ...
        if (
          exam &&
          (data.pointsObtained < 0 || data.pointsObtained > exam.totalPoints)
        ) {
