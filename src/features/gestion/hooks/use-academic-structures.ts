...
interface UpdateAcademicStructureData
  extends Partial<CreateAcademicStructureData> {
  id: string;
}

export function useAcademicStructures(
  options: UseAcademicStructuresOptions = {},
) {
    ...
    try {
      ...
      setError(
        err instanceof Error
          ? err.message
          : "Erreur lors du chargement des structures académiques",
      );
    } finally {
      setLoading(false);
    }
  ...
  const createAcademicStructure = useCallback(
    async (data: CreateAcademicStructureData): Promise<AcademicStructure> => {
      setLoading(true);
      setError(null);

      try {
        // Vérifier la contrainte : 1 structure max par professeur
        const existingStructure = academicStructures.find(
          (s) => s.createdBy === (teacherId || "current-user"),
        );
        if (existingStructure) {
          throw new Error(
            "Vous avez déjà une structure académique. Vous ne pouvez en avoir qu'une seule.",
          );
        }
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erreur lors de la création de la structure académique";
  ...
  const updateAcademicStructure = useCallback(
    async (data: UpdateAcademicStructureData): Promise<AcademicStructure> => {
      setLoading(true);
      setError(null);
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erreur lors de la mise à jour de la structure académique";
        setError(errorMessage);
        throw new Error(errorMessage);
      } finally {
        setLoading(false);
  ...
  const deleteAcademicStructure = useCallback(
    async (id: string): Promise<void> => {
      ...
      try {
        ...
        const errorMessage =
          err instanceof Error
            ? err.message
            : "Erreur lors de la suppression de la structure académique";
        setError(errorMessage);
        throw new Error(errorMessage);
      } finally {
        setLoading(false);
  ...
  const getStructureById = useCallback(
    (id: string): AcademicStructure | undefined => {
  ...
  const getTeacherStructure = useCallback((): AcademicStructure | undefined => {
    return academicStructures.find(
      (structure) => structure.createdBy === (teacherId || "current-user"),
    );
  ...
  const validateStructure = useCallback(
    (
      data: CreateAcademicStructureData,
      ...
      if (
        !data.periodsPerYear ||
        data.periodsPerYear < 1 ||
        data.periodsPerYear > 12
      ) {
      ...
      if (
        !data.periodNames ||
        Object.keys(data.periodNames).length !== data.periodsPerYear
      ) {
        errors.push(
          "Le nombre de noms de périodes doit correspondre au nombre de périodes",
        );
      }
  ...
  const importStructure = useCallback(
    async (jsonData: string): Promise<AcademicStructure> => {
      try {
