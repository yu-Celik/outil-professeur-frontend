...
import { StudentProfileService, type ProfileGenerationParams } from "../services/student-profile-service";
...
export interface UseStudentProfileGenerationReturn {
  ...
  // Actions génération
  generateProfile: (params?: ProfileGenerationParams) => Promise<StudentProfile | null>;
  regenerateProfile: (profileId: string, params?: ProfileGenerationParams) => Promise<StudentProfile | null>;
  updateProfile: (profileId: string, data: Partial<StudentProfile>) => Promise<StudentProfile | null>;

  // Actions validation
  validateProfile: (profileId: string, notes: string) => Promise<StudentProfile | null>;
  reviewProfile: (profileId: string, feedback: string) => Promise<void>;

  // Export
  exportProfile: (profileId: string, format: 'pdf' | 'json') => Promise<string | null>;
...
export function useStudentProfileGeneration({
  ...
  const loadExistingProfiles = useCallback(async () => {
    try {
      const existingProfiles = await StudentProfileService.getProfilesByStudent(studentId, academicPeriodId);
      setProfiles(existingProfiles);

      // Sélectionner le profil le plus récent comme profil courant
      const latestProfile = existingProfiles
  ...
  const generateProfile = useCallback(async (params: ProfileGenerationParams = {}): Promise<StudentProfile | null> => {
    setIsLoading(true);
    setError(null);
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : "Erreur lors de la génération du profil";
  ...
  const regenerateProfile = useCallback(async (
    ...
    try {
      const updatedProfile = await StudentProfileService.regenerateProfile(profileId, params);
      ...
      const errorMessage = err instanceof Error ? err.message : "Erreur lors de la régénération du profil";
      setError(errorMessage);
  ...
  const updateProfile = useCallback(async (
    ...
    try {
      const updatedProfile = await StudentProfileService.updateProfile(profileId, data);
  ...
  const validateProfile = useCallback(async (
    ...
    try {
      ...
      // Mettre à jour l'état local
      setProfiles(prev => prev.map(p => p.id === profileId ? validatedProfile : p));
      ...
      const errorMessage = err instanceof Error ? err.message : "Erreur lors de la validation du profil";
      setError(errorMessage);
  ...
  const reviewProfile = useCallback(async (
    ...
    try {
      ...
      const errorMessage = err instanceof Error ? err.message : "Erreur lors de la review du profil";
      setError(errorMessage);
  ...
  const exportProfile = useCallback(async (
    ...
    try {
      const exportedData = await StudentProfileService.exportProfile(profileId, format);
      return exportedData;
    } catch (err) {
      const errorMessage = err instanceof Error ? err.message : "Erreur lors de l'export du profil";
      setError(errorMessage);
  ...
  const getProfilesByPeriod = useCallback((periodId: string): StudentProfile[] => {
    return profiles.filter(profile => profile.academicPeriodId === periodId);
  }, [profiles]);

  const getProfilesByStatus = useCallback((status: string): StudentProfile[] => {
