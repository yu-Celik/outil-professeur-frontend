...
const createParticipation = (
  ...
  return {
    ...
    markAttendance: function (isPresent: boolean) {
      this.isPresent = isPresent;
    },
    setParticipationLevel: function (level: number) {
      this.participationLevel = level;
    },
    addRemarks: function (remarks: string) {
      this.specificRemarks = remarks;
    },
    updateBehavior: function (behavior: string) {
...
MOCK_COMPLETED_SESSIONS.forEach((session) => {
  const students = MOCK_STUDENTS.filter(
    (s) => s.currentClassId === session.classId,
  );
  students.forEach((student) => {
    MOCK_STUDENT_PARTICIPATION.push(
      createParticipation(student.id, session.id),
    );
  });
});
