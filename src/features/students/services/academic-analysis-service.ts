...
export class AcademicAnalysisService {
  ...
  static analyzeSubjectPerformances(
    ...
    Object.entries(resultsBySubject).forEach(([subjectId, results]) => {
      ...
      const analysis = this.analyzeSubjectResults(
        subjectId,
    ...
    const sortedSubjects = Object.entries(subjectAnalyses).sort(
   ...
   */
  static calculateAcademicProgress(
    examResults: StudentExamResult[],
  ): AcademicProgress {
  ...
  static detectAcademicRisks(
    ...
    const performances = this.analyzeSubjectPerformances(
      examResults,
      exams,
      subjects,
    );
    ...
    if (performances.consistencyScore < 0.6) {
      risks.push({
        ...
        affectedSubjects: Object.values(performances.subjectAnalyses)
          ...
          .map((analysis) => analysis.subjectName),
        description:
          "Performances irrégulières entre les matières ou dans le temps",
        suggestedActions: [
  ...
  static generateAcademicRecommendations(
    ...
    if (analysis.overallAverage < 10) {
      recommendations.push({
        ...
        description:
          "Les résultats globaux nécessitent un accompagnement spécialisé",
        priority: "high",
        actionItems: [
  ...
  private static calculateConsistencyScore(subjectAverages: number[]): number {
    ...
    const variance =
      subjectAverages.reduce((sum, avg) => sum + Math.pow(avg - mean, 2), 0) /
      subjectAverages.length;
    const standardDeviation = Math.sqrt(variance);
  ...
  private static calculateOverallTrend(
    ...
    const firstAvg =
      firstHalf.reduce((sum, grade) => sum + grade, 0) / firstHalf.length;
    const secondAvg =
      secondHalf.reduce((sum, grade) => sum + grade, 0) / secondHalf.length;
  ...
  private static identifyAcademicMilestones(
    results: StudentExamResult[],
  ): AcademicMilestone[] {
