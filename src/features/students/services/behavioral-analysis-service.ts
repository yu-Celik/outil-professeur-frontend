...
export class BehavioralAnalysisService {
  ...
  static analyzeBehavioralPatterns(
    ...
    const avgParticipation = this.calculateAverageLevel(
      validParticipations,
      "participationLevel",
    );
    ...
    const attendanceRate =
      totalSessions > 0 ? presentSessions / totalSessions : 0;
    ...
    const behavioralEvolution =
      this.calculateBehavioralEvolution(participations);
   ...
   */
  static calculateBehavioralTrends(
    participations: StudentParticipation[],
  ): BehavioralTrends {
    ...
    const firstPeriodParticipation = this.calculateAverageLevel(
      firstPeriod.filter((p) => p.isPresent),
      "participationLevel",
    );
    const secondPeriodParticipation = this.calculateAverageLevel(
      secondPeriod.filter((p) => p.isPresent),
      "participationLevel",
    );

    // Calculer les tendances
    const attentionTrend = this.calculateTrend(
      firstPeriodAttention,
      secondPeriodAttention,
      "attention",
    );
    const participationTrend = this.calculateTrend(
      firstPeriodParticipation,
      secondPeriodParticipation,
      "participation",
    );
    ...
    return {
      attentionTrend,
      participationTrend,
      evolutionSummary: this.generateEvolutionSummary(
        attentionTrend,
        participationTrend,
      ),
    };
  ...
  static detectBehavioralAlerts(
    ...
    if (
      trends.attentionTrend.direction === "down" &&
      trends.attentionTrend.changePercentage < -20
    ) {
      alerts.push({
   ...
   */
  static generateBehavioralRecommendations(
    analysis: BehavioralFeatures,
  ): Recommendation[] {
    ...
    if (analysis.attentionLevel < 12) {
      recommendations.push({
        ...
        description:
          "L'élève montre des signes de difficulté à maintenir son attention",
        priority: "high",
        actionItems: [
  ...
  private static calculateAverageLevel(
    participations: StudentParticipation[],
    field: keyof StudentParticipation,
  ): number {
  ...
  private static calculatePunctualityRate(
    participations: StudentParticipation[],
  ): number {
    if (participations.length === 0) return 0;
  ...
  private static calculateCameraUsageRate(
    participations: StudentParticipation[],
  ): number {
    if (participations.length === 0) return 0;
  ...
  private static identifyDominantBehaviors(metrics: {
    ...
    else if (metrics.cooperation <= 8)
      behaviors.push("Difficultés relationnelles");
  ...
  private static calculateBehavioralEvolution(
    participations: StudentParticipation[],
    ...
    const firstAvg = this.calculateOverallAverage(
      firstHalf.filter((p) => p.isPresent),
    );
    const secondAvg = this.calculateOverallAverage(
      secondHalf.filter((p) => p.isPresent),
    );
  ...
  private static generateEvolutionSummary(
    ...
    if (attentionTrend.direction !== "stable") {
      trends.push(
        `attention ${attentionTrend.direction === "up" ? "en amélioration" : "en baisse"}`,
      );
    }

    if (participationTrend.direction !== "stable") {
      trends.push(
        `participation ${participationTrend.direction === "up" ? "en amélioration" : "en baisse"}`,
      );
    }
