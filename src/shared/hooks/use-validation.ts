...
export type ValidationRule<T> = (value: any, items: T[], excludeId?: string) => string | null;

/**
 * Créer une règle de validation pour champ obligatoire
 */
export function requiredRule<T>(fieldName: string, displayName: string): ValidationRule<T> {
  return (value: any) => {
...
export function validateObject<T, FormData>(
  ...
  for (const [field, rules] of Object.entries(fieldRules) as Array<[keyof FormData, ValidationRule<T>[]]>) {
    errors[field] = validateWithRules((formData as any)[field], rules, items, excludeId);
  }
