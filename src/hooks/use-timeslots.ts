...
export function useTimeSlots(teacherId: string = "KsmNtVf4zwqO3VV3SQJqPrRlQBA1fFyR") {
  const [timeSlots, setTimeSlots] = useState<TimeSlot[]>(
    MOCK_TIME_SLOTS.filter(slot => slot.createdBy === teacherId)
  ...
  const validateTimeSlot = useCallback((data: CreateTimeSlotData): string[] => {
    const errors: string[] = [];
  ...
  const createTimeSlot = useCallback(async (data: CreateTimeSlotData): Promise<TimeSlot> => {
    setLoading(true);
    ...
    try {
      ...
      const duration = calculateDuration(data.startTime, data.endTime);
      const nextOrder = Math.max(...timeSlots.map(slot => slot.displayOrder), 0) + 1;
  ...
  // Modifier un créneau
  const updateTimeSlot = useCallback(async (data: UpdateTimeSlotData): Promise<TimeSlot> => {
    setLoading(true);
  ...
  const reorderTimeSlots = useCallback(async (reorderedSlots: TimeSlot[]): Promise<void> => {
    setLoading(true);
  ...
  const checkConflicts = useCallback((newSlot: CreateTimeSlotData | UpdateTimeSlotData, excludeId?: string) => {
